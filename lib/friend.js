    ...
    userProvide.findOne({nickName: nickName}, {}, function (err, userInfo) {
        if (err) {
            ...
            if (userInfo == null || userInfo == undefined) {
                ...
                userInfo.findOne({nickName: friendName}, {}, function (err, friendInfo) {
                    if (err) {
                        callback({code: 5000, message: "db is err!"});
                    } else {
                        if (friendInfo == null || friendInfo == undefined) {
                            friendProvide.findOne({nickName: nickName}, {}, function (err, userFriendInfo) {
                                if (err || userFriendInfo == null) {
                                    callback({code: 5000, message: "db is err!"});
                                } else {
                                    var friendList = userFriendInfo.friends;
                                    var flag = 0;
                                    for (var index in friendList) {
                                        if (friendList[index].nickName == friendName) {
                                            flag = 1;
                                            break;
                                        }
                                    }
                                    if (flag == 0) {
                                        callback({code: 3200, message: "该用户不是你的好友"});
                                    } else {
                                        friendProvide.update({nickName: nickName}, {"$pull": {"friends.$.nickName": "friendName"}}, function (err, result) {   //需测试是否将friendName好友删除了
                                            if (err) {
                                                callback({code: 5000, message: "db is err!"});
                                            } else {
                                                callback({code: 0});
